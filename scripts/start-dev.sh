#!/bin/bash

# ==========================================
# CarFlow SaaS - Development Start Script
# ==========================================

set -e

echo "🔧 Starting CarFlow SaaS in DEVELOPMENT mode..."
echo ""

# Check if .env exists
if [ ! -f .env ]; then
    echo "📝 Creating .env file from .env.example..."
    cp .env.example .env
fi

# Check if Docker is running
if ! docker info > /dev/null 2>&1; then
    echo "❌ Docker is not running. Please start Docker and try again."
    exit 1
fi

echo "🐳 Starting Docker containers with dev tools..."
docker-compose -f docker-compose.yml -f docker-compose.dev.yml up -d

echo ""
echo "⏳ Waiting for services to be ready..."
sleep 5

# Wait for postgres
echo "📊 Waiting for PostgreSQL..."
until docker-compose exec -T postgres pg_isready -U carflow > /dev/null 2>&1; do
    echo "   PostgreSQL is starting up..."
    sleep 2
done
echo "✅ PostgreSQL is ready!"

# Run migrations
echo ""
echo "🔄 Running database migrations..."
docker-compose run --rm migrate

echo ""
echo "✅ CarFlow SaaS Development Environment is running!"
echo ""
echo "📍 Application:"
echo "   API:            http://localhost:8080"
echo "   Health Check:   http://localhost:8080/healthz"
echo "   API Docs:       http://localhost:8080/api-docs"
echo "   Metrics:        http://localhost:8080/metrics"
echo ""
echo "🛠️  Development Tools:"
echo "   PgAdmin:        http://localhost:5050"
echo "     Email:        admin@carflow.local"
echo "     Password:     admin"
echo ""
echo "   Redis Commander: http://localhost:8081"
echo ""
echo "   MailHog:        http://localhost:8025"
echo "     SMTP:         localhost:1025"
echo ""
echo "💾 Database:"
echo "   PostgreSQL:     localhost:5432"
echo "   User:           carflow"
echo "   Password:       carflow123"
echo "   Database:       carflow"
echo ""
echo "📊 Demo Data:"
echo "   Tenant:         demo-dealership"
echo "   Email:          owner@demo-dealership.com"
echo "   Password:       password123"
echo ""
echo "📝 View logs: docker-compose -f docker-compose.yml -f docker-compose.dev.yml logs -f"
echo "🛑 Stop: docker-compose -f docker-compose.yml -f docker-compose.dev.yml down"
echo ""
